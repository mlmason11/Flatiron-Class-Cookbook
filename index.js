...
const rateFormContainer = document.getElementById('rate-form-container')
...
function populateDetails(recipeObj) {
    ...
    const avgStars = document.createElement('p')
    const totalRatings = document.createElement('p')
    if (recipeObj.ratings === 0) {
        avgStars.textContent = `This recipe has not been rated yet`
        rating.append(avgStars)
    }
    else {
        
        avgStars.textContent = `${recipeObj.stars / recipeObj.ratings} Stars`
        totalRatings.textContent = `${recipeObj.ratings} Rating(s)`
        rating.append(avgStars, totalRatings)
    }
    ...
    ratingSubmit.setAttribute('class', 'rate-button')
    ratingSubmit.setAttribute('value', 'Rate')
    ...
    ratingForm.addEventListener('submit', e => {
        ...
        else if (recipeObj.ratings === 1) {
            avgStars.textContent = `${recipeObj.stars} Stars`
            totalRatings.textContent = `${recipeObj.ratings} Rating`
        }
        else {
            avgStars.textContent = `${recipeObj.stars / recipeObj.ratings} Stars`
            totalRatings.textContent = `${recipeObj.ratings} Ratings`
        }
    })
    rateFormContainer.append(ratingForm)
...
commentForm.addEventListener('submit', e => {
    ...
    if (currentRecipe.id !== undefined) {
        ...
        .then(commentObj => {return(commentObj)})
        .catch(error => alert(error))
    }
    else {
